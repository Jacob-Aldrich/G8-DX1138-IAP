...
void Scenarios::ChooseRandomEvent(Game* game)
{
    ...
    if (game->isRunning && !game->deathEndingTriggered)
    {
    ...
    if (scenario == 0)
    {
        ...
        if (answer == 'y' || answer == 'Y')
        {
            ...
            else
            {
                ...
                std::string survivorName = survivor->GetName();
                ConsoleColor::Print(survivorName, ConsoleColor::BRIGHT_CYAN);
                std::cout << " was attacked and took " << damage << " damage.\n";
                if (!survivor->IsAlive())
                {
                    game->RemoveDeadSurvivor(survivor);
                }
            }
        }
        else
...
void Scenarios::RunChooseSomeoneEvent(Game* game)
{
    ...
    else
    {
        ...
        else
        {
            ...
            survivor->TakeDamage(damage);
            std::string survivorName = survivor->GetName();
            std::cout << "The trader was hostile. " << survivorName
